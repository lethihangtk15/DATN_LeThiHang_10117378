...
<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Sửa Video</h1>
			</div>
		</div><!--/.row-->
								...
								<div class="col-xs-8">
									...
									<div class="form-group">
										...
										<select required="" name="cate" class="form-control">
											...
											<option value="{{$type->id}}" @if($video->idChuyenMuc==$type->id) selected @endif>{{$type->tenchuyenmuc}}</option>
											@endforeach
					                    </select>
									</div>
									<div class="form-group" >
										<label>Tiêu đề </label>
										<input required type="text" name="name" class="form-control" value="{{$video->TieuDe}}">
									</div>
									<div class="form-group" >
											
                                    <label>Link</label>
										<input required type="text" name="link" class="form-control" value="{{$video->Link}}">
									</div>
									<div class="form-group" >
										<label>Tóm tắt</label>
										<textarea class="ckeditor" required name="tomtat">{{$video->TomTat}}</textarea>
									...
									<div class="form-group" >
										...
										<textarea class="ckeditor" required name="noidung">{{$video->NoiDung}}</textarea>
										<script type="text/javascript">
											var editor=CKEDITOR.replace('description',{
												language:'vi',
												filebrowserImageBrowseUrl: 'backend/ckfinder/ckfinder.html?Type=Images',
